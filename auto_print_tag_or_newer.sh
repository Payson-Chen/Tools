...
function current_tag_or_newer_commit()
{
    if git describe --exact-match --tags $(git log -n1 --pretty='%h') >/dev/null 2>&1; then
        
        latest_tag=$(get_tag)
        echo "[current_tag_or_newer_commit]:最新提交是一个标签（${latest_tag}）$1。"
        return 0
    else
        latest_tag=$(get_tag)
        echo "[current_tag_or_newer_commit]:最新提交不是标签（${latest_tag}）$1。"
#        pod push
#        latest_tag=$(get_tag)
        echo "[current_tag_or_newer_commit]:打包后的标签（${latest_tag}）$1。"
        return 1
    fi
}
...
#        pod push $2
#    fi
    current_tag_or_newer_commit $dir
    result=$?
...
#        pod_name=${dir%%/*}
        ...
        newerPods="$newerPods pod \"$pod_name\",'$latest_tag' \n"
        
        
#        echo "变量是 true。"$dir
#        echo "变量是 true。"$tmp
       # echo "变量是 true。"$newerPods

    else
        #echo "变量是 false。"
        usedTaged="$usedTaged pod \"$pod_name\",'$latest_tag' \n"
    fi
...
echo "------------------------"
echo "allTaged（当前所有组件的Tags）： = \n"$allTaged
echo "------------------------"

echo "*********************************************"

echo "------------------------"
echo "usedTaged（已经发版的所有组件的Tags）： = \n"$usedTaged
echo "------------------------"

echo "------------------------"
echo "newerPods（不是最新Tag组件）： = \n"$newerPods
echo "------------------------"
